...
;(function ($,window,document,undefined) 
{
    ...
    // Create the defaults once 디폴트 값 생성
    var pluginName = 'chromaGallery',
        ...
        keys = 
        {
            next:39, //right arrow  우측 화살표
            prev:37, //left arrow  좌측 화살표
            fullscreen:70, //letter f  다음 f 
            close:27 //escape key  탈출(닫기) 버튼
        };

    //Get the browser specific vendor prefix  특정 브라우저 설정
    ...
    //Get rgb color from color name 새상 이름에서 RGB 색상 가져오기
    ...
    //Get filename from url URL에서 파일이름 가져오기
    function getUrlFilename(url)
    {
        if(typeof url === 'string')
        {
            //remove query strings 쿼리 문자열 지우기
            return url.split(/[?#]/)[0].split("/").pop();
        }
        return undefined;
    }

    //Validate user settings  사용자 설정 유효성 검사
    ...
    // The actual plugin constructor  실제 플러그인 생성자
    ...
    $.extend(Plugin.prototype,
    {
        ...
        //Initialize the gallery grid  갤러리 그리드 초기화
        _init:function()
        {
            var self = this;
            ...
            setTimeout(function()
            {
                ...
                ({
                    load: function()
                    {
                        ...
                        {
                            $img.css({'height':'160px',"opacity":"0"})
                                .parent().attr("data-isloaded","false").css({'cursor':'default','opacity':"1"})
                                .find(".chrg-description").css({"opacity":"1"})
                                .find("p").append("<span style='line-height:70px;'>?</span><br>"+" 해당 이미지를 <br>찾을 수 없습니다.");
                        }
                        else
                        {
                    ...
                    error: function()
                    {
                        ...
                        $img.css({'height':'160px',"opacity":"0"})
                            .parent().attr("data-isloaded","false").css({'cursor':'default','opacity':"1"})
                            .find(".chrg-description").css({"opacity":"1"})
                            .find("p").append("<span style='width:100%;'><img src='/images/icon_gallery_g.png' style='width:60px; height:60px; margin:4px auto;'></span><br>"+" 해당 이미지를 <br>찾을 수 없습니다.");
                        self.grid.masonry('layout');
                    }
                }).each(function(i) 
                { 
        ...
        //Get the percentage of column width  열 너비의 비율 가져 오기
        ...
        //Create the gallery grid  갤러리 그리드 만들기
        _createGrid:function()
        {
            ...
            if(popSelect == 1){
	            var links = new Array();
	            for (i = 0; i < this.$el.find("img").length; i++) {
	                links.push( this.$el.find("img").eq(i).data("imghref"));
	            }
	            for (i = 0; i < this.$el.find("img").length; i++) {
	            this.grid.find('.chrg-item').eq(i).wrap("<a href='"+links[i]+"'></a>");
              }
            }
        },

        //Reload the gallery grid  갤러리 그리드 새로 고침
        ...
        //Set the gallery events  갤러리 이벤트 설정
        _setEvents:function()
        {
            var self = this;
    ...
    $.extend(ChromaScreen.prototype,
    {
        ...
        init:function()
        {
        	if(popSelect==0){
	            $body.append
	            (
	                "<div class='chroma-screen'><div class='chroma-bg'></div>"+
	                    "<div class='chrg-wrap'>"+
	                        "<div class='chrg-content'>"+
	                        "<div class='chrgi-previous chrg-left-btn'><img src='/images/svg/chrg_back.svg'/></div>"+
	                        "<div class='chrgi-next chrg-right-btn'><img src='/images/svg/chrg_next.svg'/></div>"+
	                        "<div class='chrg-imgwrap' data-isloaded='false'></div></div>"+
	                        "<div class='chrg-ui'><p class='chrg-index'></p>"+
	                            "<button class='chrg-no-select'><img class='chrg-close-css' src='/images/svg/icon_x_w.svg'/></button>"+
	                            "<button class='chrg-no-select'></button>"+
        		...
        		(
        			"<div class='chroma-screen'><div class='chroma-bg'></div>"+
	                    "<div class='chrg-wrap'>"+
	                        ...
	                        "<div class='chrg-ui'><p class='chrg-index'></p>"+
	                            "<button class='chrg-no-select'><img class='chrg-close-css' src='/images/svg/icon_x_w.svg'/></button>"+
	                            "<button class='chrg-no-select'><img class='chrg-close-css' src='/images/icon_garb.png'/></button>"+
	                            "<div class='chrg-loader'>"+
	                                "<div class='chrg-loader-line-wrap-wrap'>"+
	                                    "<div class='chrg-loader-line-wrap'></div></div>"+
	                                "<div class='chrg-loader-line-wrap-wrap'>"+
